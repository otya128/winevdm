...
#if 0
void init__wine_spec_dos_header()
{
	PIMAGE_DOS_HEADER dosh = __wine_spec_dos_header;
	memset(dosh, 0, sizeof(IMAGE_DOS_HEADER));
	dosh->e_magic = IMAGE_DOS_SIGNATURE;
	dosh->e_lfanew = sizeof(IMAGE_DOS_HEADER);
	PIMAGE_OS2_HEADER ne = __wine_spec_dos_header + sizeof(IMAGE_DOS_HEADER);
	...
	//ne->ne_cbenttab = 0x00;
	ne->ne_crc = 0;
	ne->ne_flags = NE_FFLAGS_SINGLEDATA |                    /* ne_flags */
		(/*(spec->characteristics & IMAGE_FILE_DLL) ?*/ NE_FFLAGS_LIBMODULE);
	ne->ne_autodata = 2;
	//ne->ne_heap = 
	ne->ne_stack = 0;
	ne->ne_csip = 0;//FIXME:entry point
	ne->ne_sssp = 0x00000002;
	ne->ne_cseg = 2;
	ne->ne_cmod = 0;
	ne->ne_cbnrestab = 0;
	/******/
	...
	ne->ne_flagsothers = NE_AFLAGS_FASTLOAD;
	ne->ne_pretthunks = 0;
	ne->ne_psegrefbytes = 0;
	ne->ne_swaparea = 0;
	ne->ne_expver = 0;
	/* segment table */
	/* code segment entry */
	...
	segment *code = (segment*)(ne + 1);
	code->offset = 0;
	code->length = 0;
	code->flag = FLAG_32BIT;
	code->minsize = 0;
	segment *data = (segment*)(code + 1);
	data->offset = 0;
	data->length = 0;
	data->flag = DataSegment;
	data->minsize = 0;
	WORD* resource = (WORD*)(data + 1);
		//align
	*resource = 0;
	resource++;
	*resource = 0;
	resource++;
	resource = 0;
	resource = ((BYTE*)resource) + 1;
	BYTE* restab = resource;
	//.align
	restab = restab + ((size_t)restab % 2);
